<?php
/**
 * kv.php
 */
?>

<div class="kv bg-yellow" data-hd-color>
	<div class="kv-bg mask-img is-fast">
	<picture>
		<source srcset="<?php echo imgDir(); ?>/home/kv.webp?v1.0.1" type="image/webp" media="(min-width: 768px)">
		<img src="<?php echo imgDir(); ?>/home/kv-sp.webp?v1.0.1" alt="Key Visual Background">
	</picture>
	</div>
	<div class="in-lg kv-in">
	<div class="kv-content">
		<h1 class="kv-ttl">35歳からの目元に。</h1>
		<p class="kv-sub">“年齢まつ毛<span class="qe">”</span> 専用美容液、誕生。</p>
	</div>
	</div>
</div>
